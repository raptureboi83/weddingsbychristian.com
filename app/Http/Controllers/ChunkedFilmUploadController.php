...
class ChunkedFilmUploadController extends Controller
{
    public function chunk(Request $request)
    {
        try {
            ...
            Log::error('Chunked upload chunk failed: ' . $e->getMessage(), [
    ...
    public function finalize(Request $request)
    {
        try {
            ...
            if (! File::isDirectory($tempDir)) {
                Log::warning('Chunked upload finalize: temp dir not found', ['upload_id' => $uploadId]);
                return response()->json([
            ...
            for ($i = 0; $i < $totalChunks; $i++) {
                ...
                if (! File::exists($partPath)) {
                    Log::warning('Chunked upload finalize: missing chunk', ['upload_id' => $uploadId, 'chunk' => $i]);
                    return response()->json([
            ...
            if ($output === false) {
                Log::error('Chunked upload finalize: could not create output file', ['path' => $finalAbsolutePath]);
                return response()->json([
            ...
            Log::error('Chunked upload finalize failed: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
