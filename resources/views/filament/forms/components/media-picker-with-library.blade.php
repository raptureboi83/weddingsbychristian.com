<div
    x-data="mediaPickerWithLibrary({
        statePath: @js($targetStatePath ?? $livewireStatePath),
        existingPath: @js($existingPath ?? ''),
        uploadChunkUrl: @js($uploadChunkUrl ?? route('filament.admin.media.upload.chunk')),
        finalizeUrl: @js($finalizeUrl ?? route('filament.admin.media.upload.finalize')),
        listUrl: @js($listUrl ?? route('filament.admin.media.list')),
...
>
    ...
    <style>
        .media-picker-library { display:flex; flex-direction:column; gap:0.75rem; }
    ...
    <script>
        function mediaPickerWithLibrary(config) {
            return {
